...
class UM_Admin_Dashboard {
	...
	public function menu_order_count() {
		...
		if(is_array($menu)){
			foreach( $menu as $key => $menu_item ) {
				if ( 0 === strpos( $menu_item[0], _x( 'Usuarios', 'Admin menu name' ) ) ) {
		...
		if(is_array($submenu)){
			foreach ( $submenu['users.php'] as $key => $menu_item ) {
				if ( 0 === strpos( $menu_item[0], _x( 'Todos Usuarios', 'Admin menu name' ) ) ) {
	...
	***/
	function primary_admin_menu() {
		...
		$this->pagehook = add_menu_page( __('Administrar Revendedores', $this->slug), __('Administrar Revendedores', $this->slug), 'manage_options', $this->slug, array(&$this, 'admin_page'), 'dashicons-admin-users', '42.78578');
		add_action('load-'.$this->pagehook, array(&$this, 'on_load_page'));
		
		add_submenu_page( $this->slug, __('Painel de Controle', $this->slug), __('Painel de Controle', $this->slug), 'manage_options', $this->slug, array(&$this, 'admin_page') );
	...
	***/
	function secondary_menu_items() {
		...
		add_submenu_page( $this->slug, __('Formularios', $this->slug), __('Formularios', $this->slug), 'manage_options', 'edit.php?post_type=um_form', '', '' );
		add_submenu_page( $this->slug, __('Funções do Usuário', $this->slug), __('User Roles', $this->slug), 'manage_options', 'edit.php?post_type=um_role', '', '' );

		if ( um_get_option('members_page' ) || !get_option('um_options') ){
			add_submenu_page( $this->slug, __('Diretórios de Membros', $this->slug), __('Diretórios de Membros', $this->slug), 'manage_options', 'edit.php?post_type=um_directory', '', '' );
		}
	...
	***/
	function extension_menu() {
		...
		add_submenu_page( $this->slug, __('Extenções', $this->slug), '<span style="color: #00B9EB">' .__('Extenções', $this->slug) . '</span>', 'manage_options', $this->slug . '-extensions', array(&$this, 'admin_page') );
	...
	***/
	function on_load_page() {
		...
		add_meta_box('um-metaboxes-contentbox-1', __('Visão geral dos usuários','ultimate-member'), array(&$this, 'users_overview'), $this->pagehook, 'core', 'core');
		
		add_meta_box('um-metaboxes-mainbox-1', __('Últimas de nosso blog','ultimate-member'), array(&$this, 'um_news'), $this->pagehook, 'normal', 'core');
		
		add_meta_box('um-metaboxes-sidebox-1', __('Arquivos Temporarios','ultimate-member'), array(&$this, 'purge_temp'), $this->pagehook, 'side', 'core');
		add_meta_box('um-metaboxes-sidebox-2', __('Cache do Usuário','ultimate-member'), array(&$this, 'user_cache'), $this->pagehook, 'side', 'core');
