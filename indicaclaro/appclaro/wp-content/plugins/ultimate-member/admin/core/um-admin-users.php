...
class UM_Admin_Users {
	...
	***/
	function user_row_actions($actions, $user_object) {
		...
		if ( um_user('submitted') ) {
			$actions['view_info'] = '<a href="#" data-modal="UM_preview_registration" data-modal-size="smaller" data-dynamic-content="um_admin_review_registration" data-arg1="'.$user_id.'" data-arg2="edit_registration">' . __('Informações do Usuario','ultimate-member') . '</a>';
		}
	...
	***/
	function views_users( $views ) {
		...
		if ( !isset($_REQUEST[ $this->custom_role ]) && !isset($_REQUEST['status']) ) {
			$views['all'] = '<a href="'.admin_url('users.php').'" class="current">Todos <span class="count">('.$ultimatemember->query->count_users().')</span></a>';
		} else {
			$views['all'] = '<a href="'.admin_url('users.php').'">Todos <span class="count">('.$ultimatemember->query->count_users().')</span></a>';
		}

		$status = array(
			'approved' => __('Aprovados','ultimate-member'),
			'awaiting_admin_review' => __('Revendedor Pendente','ultimate-member'),
			'awaiting_email_confirmation' => __('Esperando Confirmação do E-mail','ultimate-member'),
			'inactive' => __('Inativo','ultimate-member'),
			'rejected' => __('Rejeitado','ultimate-member')
		);
		...
		if (isset($_REQUEST) && !empty ($_REQUEST) ){
			...
			if (isset($_REQUEST['users']) && is_array($_REQUEST['users']) && isset($_REQUEST['um_changeit']) && $_REQUEST['um_changeit'] != '' && isset($_REQUEST['um_change_role']) && !empty($_REQUEST['um_change_role']) ){
					...
					if ( ! current_user_can( 'edit_users' ) )
						wp_die( __( 'Você não tem permissões suficientes para fazer isso.','ultimate-member') );
	...
	function restrict_manage_users() {
		...
		?>
			...
			<div style="float:right;margin:0 4px">
				...
				<label class="screen-reader-text" for="um_filter_role"><?php _e('Filtrar Por','ultimate-member'); ?></label>
				<select name="um_filter_role[]" id="um_filter_role" class="" style="width: 120px">
					<option value="0"><?php _e('Filtrar Por','ultimate-member'); ?></option>
					<?php
				...
				<input name="um_role" id="um_role" class="button" value="<?php _e('Filtrar'); ?>" type="submit" />

			</div>

			<div style="float:right;margin:0 4px">

				<label class="screen-reader-text" for="um_bulk_action"><?php _e('Ação dos Usuarios','ultimate-member'); ?></label>
				<select name="um_bulk_action[]" id="um_bulk_action" class="" style="width: 200px">
					<option value="0"><?php _e('Ação dos Usuarios','ultimate-member'); ?></option>
					<?php echo $ultimatemember->user->get_bulk_admin_actions(); ?>
				</select>

				<input name="um_bulkedit" id="um_bulkedit" class="button" value="<?php _e('Aplicar'); ?>" type="submit" />

			</div>

			<div style="float:right;margin:0 4px">

				<label class="screen-reader-text" for="um_change_role"><?php _e('Papel Comunitário&hellip;','ultimate-member'); ?></label>
				<select name="um_change_role[]" id="um_change_role" class="" style="width: 160px">
					<?php foreach($ultimatemember->query->get_roles( $add_default = 'Papel Comunitário&hellip;' ) as $key => $value) { ?>
					<option value="<?php echo $key; ?>"><?php echo $value; ?></option>
					<?php } ?>
				</select>

				<input name="um_changeit" id="um_changeit" class="button" value="<?php _e('Trocar','ultimate-member'); ?>" type="submit" />
	...
	***/
	function manage_users_columns($columns) {
		...
		$columns['um_role'] = __('Papel da comunidade','ultimate-member') . $admin->_tooltip( __('Esta é a função de associação definida pelo complemento do Membro final','ultimate-member') );
